...
class GetMetaResult:
    ...
    def webhook_ips(self) -> Sequence[_builtins.str]:
        """
...
def get_meta(opts: Optional[pulumi.InvokeOptions] = None) -> AwaitableGetMetaResult:
    ...
    from_buildkite = aws.SecurityGroup("from_buildkite",
        name=from_buildkite,
        ingress=[{
            fromPort: *,
            toPort: 443,
...
def get_meta_output(opts: Optional[Union[pulumi.InvokeOptions, pulumi.InvokeOutputOptions]] = None) -> pulumi.Output[GetMetaResult]:
    """
    Use this data source to look up the source IP addresses that Buildkite may use to send external requests,
    including webhooks and API calls to source control systems (like GitHub Enterprise Server and BitBucket Server).
    ...
    __ret__ = pulumi.runtime.invoke_output('buildkite:index/getMeta:getMeta', __args__, opts=opts, typ=GetMetaResult)
    return __ret__.apply(lambda __response__: GetMetaResult(
        id=pulumi.get(__response__, 'id'),
        webhook_ips=pulumi.get(__response__, 'webhook_ips')))
